...
int graph::game(std::ifstream &input) {
    ...
    adj_list::adj_node * list = new adj_list::adj_node;
    adj_list::adj_node * list_node = list;
    for (int i = 0; i < v_size; i++) {
        if (!verts[i].deg_out) {
            list_node = list;
            ...
            list_node->vert = &verts[i];
            list_node->next = new adj_list::adj_node;
        }
    }
    ...
    while(list_node != nullptr && list_node->vert != nullptr) {
        while(!queue.empty())
            queue.pop();
        vert = list_node->vert;
        vert->color = 2;
