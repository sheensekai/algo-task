...
void item_del(avl_tree *tree, int val, vertex *vptr);

item * tree_exmin(avl_tree *tree);

void item_balance(avl_tree *tree, item *par, int mode);

void item_hb_recalc(item *par);

item *item_search(avl_tree *tree, int val, vertex *vptr);
...
void item_add(avl_tree *tree, int val, vertex * vptr) {
    // find the parent
    item *par = item_search(tree, val, vptr);
    // empty tree case
    if (par == nullptr) {
        tree->size = 1;
        tree->root = new item;
...
void item_del(avl_tree *tree, int val, vertex *vptr) {
    // find the item
    item *node = item_search(tree, val, vptr);
    ...
    if (node->left == nullptr && node->right == nullptr) {
        ...
        vertex * tmp_vert = r_node->vert;
        item *check = nullptr;
        // r_node->left case
        if (r_node->left != nullptr) {
            r_node->val = r_node->left->val;
            r_node->vert = r_node->left->vert;
            delete r_node->left;
            r_node->left = nullptr;
            check = r_node;
        }
...
item * tree_exmin(avl_tree *tree) {
    ...
    while (node->left != nullptr)
        node = node->left;
    item_del(tree, node->val, node->vert);
    return node;
}
...
item *item_search(avl_tree *tree, int val, vertex * vptr) {
    ...
    while (node != nullptr) {
        if (val < node->val && node->left != nullptr)
            ...
            node = node->right;
        else if (val == node->val && vptr != node->vert && node->left != nullptr)
            node = node->left;
        else break;
    }
